    ...
    <div className="flex flex-col lg:flex-row gap-8 min-h-[600px]">
      ...
      <div className="lg:w-80 space-y-4">
        <div className="flex items-center justify-between px-2">
           <h3 className="text-[10px] font-light text-slate-400 uppercase tracking-[0.2em]">Active Channels</h3>
           <span className="bg-blue-600 text-[10px] font-light text-white px-2 py-0.5 rounded-lg shadow-lg shadow-blue-500/10">{sessions.length} Live</span>
        </div>
        <div className="space-y-3 max-h-[700px] overflow-y-auto pr-2 custom-scrollbar">
          {sessions.length > 0 ? sessions.map((session) => (
              ...
              <div className="flex items-center gap-4 relative z-10">
                <div className={cn(
                 ...
                 <div className="flex-1 min-w-0">
                    <span className={cn(
                      "text-sm font-light tracking-tight block truncate transition-colors",
                    ...
                    <div className="flex items-center gap-2 mt-1">
                       ...
                       <span className="text-[10px] font-light text-slate-400 uppercase tracking-widest">{(session.stage || 'IDLE').replace(/_/g, ' ')}</span>
                    </div>
                    <span className="text-[9px] text-slate-300 font-light mt-0.5 block">{timeAgo(session.updated_at)}</span>
                 </div>
              </div>
            ...
            <div className="glass-card rounded-[2rem] p-10 text-center border-dashed border-2 bg-white/10">
               <span className="text-[10px] font-light text-slate-400 uppercase tracking-widest">No Active Sessions</span>
            </div>
          )}
        </div>
      </div>
          ...
          {activeSession ? (
            ...
            >
                 ...
                 <div className="flex items-center gap-6">
                    ...
                    <div className="flex flex-col gap-1">
                       <div className="flex items-center gap-2">
                          <h4 className="text-2xl font-light text-slate-900 font-outfit tracking-tight">AI Bot <span className="text-blue-600 font-medium">Active</span></h4>
                          <span className="px-2 py-0.5 bg-green-50 text-green-600 border border-green-100 rounded-lg text-[9px] font-light uppercase tracking-widest">Connected</span>
                       </div>
                        <p className="text-xs font-light text-slate-400 uppercase tracking-widest">Phone: {activeSession.phone} • Last active: {timeAgo(activeSession.updated_at)}</p>
                    </div>
                 </div>
                 <button className="p-3 bg-white border border-white rounded-2xl text-slate-400 hover:text-slate-900 transition-all shadow-sm">
                  ...
                  {[
                   ...
                   >
                      ...
                      <div className="flex flex-col gap-1.5 pt-1">
                         <span className={cn(
                           "text-base font-light uppercase tracking-tight font-outfit",
                           step.type === 'active' ? "text-blue-600 font-medium" : "text-slate-900"
                         )}>{step.label}</span>
                         <span className="text-[10px] font-light text-slate-400 uppercase tracking-[0.2em]">{step.time}</span>
                      </div>
                   </motion.div>
                 ))}
              </div>
              ...
              <div className="mt-12 p-6 rounded-[2rem] bg-indigo-50/50 border border-indigo-100/50 flex items-center justify-between">
                 <div className="flex flex-col gap-1">
                    <span className="text-[10px] font-light text-indigo-600 uppercase tracking-widest">Bot Intelligence</span>
                    <p className="text-sm font-light text-slate-900 tracking-tight">Intervene if client seems confused?</p>
                 </div>
                 <div className="flex gap-4">
                    <button className="px-6 py-3 bg-white border border-white rounded-[1.2rem] text-[10px] font-light text-slate-900 uppercase tracking-widest hover:bg-slate-50 transition-all shadow-sm">Restart Bot</button>
                    <button className="px-6 py-3 bg-indigo-600 rounded-[1.2rem] text-[10px] font-light text-white uppercase tracking-widest hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-500/20">Manual takeover</button>
                 </div>
              </div>
            </motion.div>
          ) : (
            <div className="glass-panel rounded-[3.5rem] h-full flex flex-col items-center justify-center text-center p-20 border-white/50 bg-white/20 border-dashed border-4">
               ...
               <h4 className="text-2xl font-light text-slate-300 font-outfit uppercase tracking-widest">Deep Sleep Mode</h4>
               <p className="max-w-xs text-sm text-slate-400 font-light mt-4 leading-relaxed uppercase tracking-tight">Your AI Bot is waiting for the next WhatsApp inquiry. Sessions will ignite here in real-time.</p>
            </div>
          )}
        </AnimatePresence>
      </div>
